...
```
用户输入 → 项目经理 → 市场分析师 → 营销战略师(3 方案) → 内容创作师(3 创意) → 内容审核员 → 最终方案(.docx)
```

6 个 Agent 按顺序协作，上游输出作为下游输入。市场分析师和营销战略师可按需进行网络搜索（ReAct 模式）。
...
| 💡 内容创作师 | Content Creator | 策划 3 个活动创意 + 撰写对应营销文案 |
| ✏️ 内容创作师 | Content Creator | 为每个活动撰写营销文案 |
